  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL}/auth/login?error=oauth_error&provider=google&message=${encodeURIComponent(errorDescription || error)}`
      )
    }
    ...
    if (!code || !state) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL}/auth/login?error=invalid_oauth_params&provider=google`
      )
    }

    // Handle OAuth callback
    const { user, sessionToken } = await oauthService.handleOAuthCallback('google', code, state)
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL}/auth/login?error=${errorCode}&provider=google&message=${encodeURIComponent(errorMessage)}`
    )
  }
}
